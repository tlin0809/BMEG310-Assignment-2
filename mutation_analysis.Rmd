...
```{r}
cluster <-  as.data.frame(cutree(res$tree_col, k = 2)) #JUST TWO GROUPS FOR NOW
...
clin_df$MONTHS_LAST_FOLLOWUP <- clin_df$DAYS_LAST_FOLLOWUP/30.417 #days to month conversion

#clin_df$deceased = clin_df$OS_STATUS == "1:DECEASED"

#the "deceased" variable doesnt actually correlate with death. just tumor progression
...
most_mutated <- mat["VHL", ] == 1

# Extract patient IDs where VHL gene is 1
...
#ie fit that model! 
#fit = survfit(Surv(overall_survival, deceased) ~ BAP1_Mutation, data=clin_df)
