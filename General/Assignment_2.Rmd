...
```
```{r}
confusion.matrix.v2 <- table(predicted.diagnosis, expected.diagnosis)
confusion.matrix.v2
...
for(i in 1:10){
  kmeans2 <- kmeans(ovarian.features, 2)
...
```{r}
# #computing mean of each variable by clusters using the original data
# 
# aggregate()
```
...
```{r}
# # before clustering, need to scale the data. only scaling the subset of data containing the features
# ovarian.features <- ovarian.dataset[,c(3:32)]
# 
# #code from R documentation: 
# cl <- kmeans(ovarian.features, 2)
# # plot(ovarian.features, col= cl$cluster)
# #points(cl$centers, col = 1:2, pch=8, cex=2)
# 
# ss <- function(ovarian.features) sum(scale(ovarian.features, scale = FALSE)^2)
# 
# ## cluster centers "fitted" to each obs.:
# fitted.ov.features <- fitted(cl)
# resid.ov.features <- ovarian.features - fitted(cl)
# 
# kmeans(ovarian.features,2)$withinss
