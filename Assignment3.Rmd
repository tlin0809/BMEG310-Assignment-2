...
*65.7 is the mean BQ for reads aligning to chrom X.*
```{r}
#HELPPP TA WHERE DO WE SUBTRACT 33 / DO WE NEED TO???- asked on piazza
Xchrom <- sam[sam$V3 == "X", ]
...
*asdf*
```{r}
#HELPPPPPPPPPPPP SOS TA!
bq_int <- sapply(sam$V11, utf8ToInt)-33
bq_int <- t(bq_int)

dim(bq_int)

boxplot(bq_int[,1:10], at = 1:10, xlim = c(1,58), xaxt = 'n')
...
*61,527 reads.*
```{r}
#*** CONFIRM ITS OUT OF THE WHOLE SAM FILE, NOT CHROMOSOME 9 - asked on piazza
...
## Q5.1 For the first variant (row) in the dataframe, what is the reference allele base at the
...
```{r}
#split the string with ANN=

ANN_1 = unlist(strsplit(as.character(variants$INFO)[1], "ANN="))[2]
# BUUUUT i  didnt use grep - use teams code 
...
strsplit(as.character(ANN_1), ",")

# SOS IS IT ENOUGH TO JUST SAY INTRONIC VARIANT
```
...
```
*Rps19 gene. We can see it is this gene from the annotations printed above.* (TA said this was fine, non need to extract -> maya)
...
-------> overall number aka the number of rows??
...
length(intronic_variants)
length(intergenic_variants)
total_interon_and_intergenic_variants <- length(intronic_variants) + length(intergenic_variants)
total_interon_and_intergenic_variants
nrow(variants) #overall number of variants
```
*We know that a majority of the variants are of the intronic/intergenic category* (TA liked)
...
```
*LIST OUT THE GENES WE PRINTED HEHE*

Q5.9. (bonus): Using Strelka on our data, we can detect indels, but only to a limited extent. Most
...
length(VAF_greater5[grep("protein_coding", variants$INFO[VAF_greater5])])

```

initiate an empty VAF vector 

mega super duper for loop:
split by : for format and askhdfjkdhsf
unlist both of those
use the AD index to pull the pair from akdshfkjdsfh
split the pair by ,
then do the math x/(x+y) (this is a %)
store it in VAF

1 box plot for the 836 variants that describes the distribution 


for >5% can use which probs?
use grep for: "How many of those variants (the ones with >5% VAF) are in coding regions?"